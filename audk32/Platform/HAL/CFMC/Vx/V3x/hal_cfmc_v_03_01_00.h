...
#ifndef _HAL_CFMC_V_03_01_00_H_
#define _HAL_CFMC_V_03_01_00_H_
...
#if defined (CFMC_CFGR_WRITE_Pos)
...
#define SET_CFMC_CR_SYSFLASH_LOCK(Peri,Data)          SET_BIT(Peri,Data,CFMC,CTRL,OTPLOCK)
...
#define GET_CFMC_STR_CRCBUSY(Peri)                    GET_BIT(Peri,CFMC,STAT,CBUSY)
...
/* CRC */
#define SET_CFMC_CRC_START_ADDR(Peri,Data)            SET_REG(Peri,Data,CHKSADDR)
#define SET_CFMC_CRC_END_ADDR(Peri,Data)              SET_REG(Peri,Data,CHKEADDR)
#define GET_CFMC_CRC_RESULT(Peri)                     GET_BIT(Peri,CFMC,CHKDOUT,CDOUT)
#define SET_CFMC_CRC_CR_BACKGROUND_MODE(Peri,Data)    SET_BIT(Peri,Data,CFMC,CHKCTRL,BGEN)
#define SET_CFMC_CRC_CR_BURST_MODE(Peri,Data)         SET_BIT(Peri,Data,CFMC,CHKCTRL,BSTEN)
#define SET_CFMC_CRC_CR_IRQ_EN(Peri,Data)             SET_BIT(Peri,Data,CFMC,CHKCTRL,CDIEN)
#define SET_CFMC_CRC_CR_RESET_CRC_DATA(Peri,Data)     SET_BIT(Peri,Data,CFMC,CHKCTRL,CDRST)
...
RAMFUNC static __inline HAL_ERR_e CFMC_Erase(volatile uint32_t un32PageAddr, CFMC_ERASE_MODE_e eEraseMode, bool bIsSystemFlash)
{
    ...
    switch (eEraseMode)
    {
        ...
        default:
            ...
            if (bIsSystemFlash)
            {
                SET_CFMC_CR_SYSFLASH_LOCK(ptRegBase, true);
            }
            else
            {
