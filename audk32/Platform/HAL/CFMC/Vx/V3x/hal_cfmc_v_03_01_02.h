...
#ifndef _HAL_CFMC_V_03_01_02_H_
#define _HAL_CFMC_V_03_01_02_H_
...
#define CFMC_FEATURE_WRITE_PROTECTION_BLOCKS          CONFIG_MAX_WPROT_BLOCKS
...
#define CFMC_FEATURE_BANK_SWITCH                      1
...
#define SET_CFMC_CFGR_WRITE_EN(Peri,Data)             SET_BIT(Peri,Data,CFMC,CONF,WRITE)
...
#define SET_CFMC_WPR_WPROT(Peri,Idx,Data)             ((Idx == 0) ? SET_REG(Peri,Data,FLS0PROT) : SET_REG(Peri,Data,FLS1PROT))
#define GET_CFMC_WPR_WPROT(Peri,Idx)                  ((Idx == 0) ? GET_REG(Peri,FLS0PROT)      : GET_REG(Peri,FLS1PROT))
...
/* Multi Bank */
#define CFMC_FEATURE_BANK_SWITCH_CMD_TYPE             2
#define CFMC_FEATURE_BANK_SWITCH_CMD_OFFSET           0x0F000600
#define CFMC_BANK_WKI_KEY                             0x5A3C0F
#define SET_CFMC_BANK_MODE(Peri,Data)                 SET_BIT_KEY_BITNAME(Peri, CFMC_BANK_WKI_KEY, BANK, Data, CFMC, BANK, BK)
#define SET_CFMC_BANK_SWITCH(Peri,Data)               SET_BIT_KEY_BITNAME(Peri, CFMC_BANK_WKI_KEY, BANK, Data, CFMC, BANK, BS)
#define GET_CFMC_BANK_MODE(Peri)                      GET_BIT(Peri, CFMC, BANK, BK)
...
RAMFUNC static __inline void CFMC_SetWriteProtect(uint32_t un32TargetBlk, bool bRequestLock)
{
    ...
    for(int i = 0 ; i < CFMC_FEATURE_WRITE_PROTECTION_BLOCKS ; i++)
    {
        un32BlkIndex = (un32TargetBlk >> (i * 16)) & CFMC_WPROT_WPROT_Mask;

        if(un32BlkIndex)
        {
            un32Reg = GET_CFMC_WPR_WPROT(ptRegBase, i);
            ...
            SET_CFMC_WPR_WPROT(ptRegBase, i, un32Reg);
        }
    }
...
RAMFUNC static __inline HAL_ERR_e CFMC_Erase(volatile uint32_t un32PageAddr, CFMC_ERASE_MODE_e eEraseMode, bool bIsSystemFlash)
{
    ...
    SET_CFMC_CFGR_WRITE_EN(ptRegBase, true);
    ...
    *(volatile uint32_t *)un32PageAddr = 0xFFFFFFFF;
    ...
    do
    {
        __NOP();__NOP();__NOP();__NOP();__NOP();
        un32Cnt++;
        if (un32Cnt > CFMC_PE_STATUS_CHECK_COUNT)
        {
            break;
        }
    ...
    switch (eEraseMode)
    {
         ...
         default:
             SET_CFMC_CFGR_WRITE_EN(ptRegBase, false);
             return HAL_ERR_PARAMETER;
    }
...
RAMFUNC static __inline void CFMC_WriteToUserInfo(volatile uint32_t un32FlashAddr, uint32_t *pun32Data, uint32_t un32Len, PRV_CMFC_USERINFO_ID_e eUserInfoID)
{
    ...
    un32FlashAddr |= (CFMC_FEATURE_FLASH_BASE_ADDR | CFMC_FEATURE_CODE_FLASH_BASE_OFFSET);
...
RAMFUNC static __inline void CFMC_EraseUserInfo(uint32_t un32PageAddr, CFMC_ERASE_MODE_e eEraseMode, PRV_CMFC_USERINFO_ID_e eUserInfoID)
{
    CFMC_Type *ptRegBase = (CFMC_Type *)CFMC_REG_BASE;
    uint32_t un32State = 0xFF;
    uint32_t un32Cnt = 0;

    un32PageAddr |= (CFMC_FEATURE_FLASH_BASE_ADDR | CFMC_FEATURE_CODE_FLASH_BASE_OFFSET);
...
RAMFUNC static __inline HAL_ERR_e CFMC_GetBankSwitchStatus(CFMC_BANK_ID_e *peBootBank, bool *pbBankSwitchDone, CFMC_BANK_ID_e *peActiveBank)
{
    ...
    if( !GET_CFMC_BANK_MODE(ptRegBase) && ((*(uint32_t *)CFMC_FEATURE_BANK_SWITCH_CMD_OFFSET) >> 8) == CFMC_BANK_WKI_KEY)
    {
